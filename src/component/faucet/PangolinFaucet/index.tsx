...
export const PangolinFaucet = () => {
  ...
  const handleLoginWithGithub = useCallback(() => {
    userInfo && pangolinClaimState.isRateLimit ? setClaimResultModalConfig({
      visible: true,
      resultInfo: {
        status: FaucetClaimResultStatus.IN_GREYLIST,
      }
    }) : window.open('/pangolin/github');
  ...
  useEffect(() => {
    if (userInfo?.isGithubOauth) {
      userInfo?.isOauthSuccess ? setClaimModalConfig({ visible: true }) : notification.info({ message: 'Authorize', description: 'Failed to authorize.' });
    }
  }, [userInfo]);
